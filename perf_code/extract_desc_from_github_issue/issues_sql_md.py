...
def execute_select(cursor):
    ...
    sql = "SELECT issues_number,repo,title,body FROM issues"  # SQL 查询语句
    try:
        cursor.execute(sql)
...
def parse_body(columns):
    ...
    for column in columns:
        ...
        l, kw_and_code_descs = get_kw_code_desc(body, title)
        len_kw_sen += l

        if len(kw_and_code_descs) != 0:
            save_csv(kw_and_code_descs, issues_number, repo, 'D:\\work\performance\\github_issues\\issues_desc\\desc_20_0102\\issues.csv',apis)
...
def get_kw_code_desc(body, title):
    '''
    提取含关键字和代码的句子
    :param body:主体
    :param title:标题
...
def save_csv(kw_and_code_descs,issues_number,repo,csv_Path,apis):
    '''
    ...
    with open(csv_Path, 'a+', encoding='gb18030',newline="") as csvfilehandler:
        ...
        for desc in kw_and_code_descs:
            writer.writerow([issues_number,repo,desc[0],desc[1],desc[2],apis])  # [question_id,desc,kw[],codelist]
