...
def executeSelect(cursor):
    ...
    sql = "SELECT comment_id,issue_number,body FROM comments"  # SQL 查询语句
    try:
        cursor.execute(sql)
...
def parseBody(columns):
    ...
    for column in columns:
        ...
        kw_code_desc = getKWDesc(body)
        savecsv(kw_code_desc,comment_id,issue_number,'G:\Performance\SO_Guid_Api\github_issues\desc\\comments.csv',apis)
        for i in kw_code_desc:
            print(comment_id, " : ",i)
...
def savecsv(kw_code_desc,comment_id,issue_number, csv_Path,apis):
    '''
    ...
    with open(csv_Path, 'a+', encoding='gb18030',newline="") as csvfilehandler:
        ...
        for desc in kw_code_desc:
            writer.writerow([comment_id, issue_number, desc[0], desc[1], desc[2],apis])  # [question_id,desc,kw[],codelist]
