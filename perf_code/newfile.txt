...
def get_commit(repo_path, line_start, line_end, file_path):
    commit = pd.DataFrame()
    GIT_LINE_LOG = r'git -C {} log -p -u -L {},{}:{}> {}'
    GIT_FILE_LOG = r'git -C {} log -p -u {}> {}'
    GIT_ID_LOG = r'git -C {} log -p -u -1 {} {}> {}'
    ...
    line_ids = exec_comm(git_line_log, os.getcwd() + '\\tem_log2' + '.txt')
    file_ids = exec_comm(git_file_log, os.getcwd() + '\\tem_log2' + '.txt')
    commi_id = [id for id in file_ids if id in line_ids]
    for id in commi_id:
        comm = GIT_ID_LOG.format(repo_path, id, file_path, os.getcwd() + '\\tem_log2' + '.txt')
        os.system(comm)
        with open(os.getcwd() + '\\tem_log2' + '.txt', 'r', encoding='UTF-8') as f:
            lines = f.readlines()
        df = split_log(lines)
        commit = commit.append(df, ignore_index=True)
    return commit
...
def get_commit_id(caveat, root_path, kws):
    ...
    for row in range(len(caveat)):
        ...
        # exec_git(line_no, file_path, version=v, repo_path=repo_path)
        # log = read_txt(os.getcwd() + '\\tem_log' + '.txt')
        # commit = split_log(log)
        commit = get_commit(repo_path, line_start, line_end, file_path)

        line_diff = get_line_diff(repo_path, line_start, line_end, caveat['path'][row])
        ...
        while index < len(commit):
            ...
            if len(commit_type) != 0:
                ...
                #     break
            index = index + 1
            if index == len(commit) and len(line_diff) != 0 and not tag_extral:
                commit = line_diff
                index = 0
                tag_extral = True
...
def filter_id(commit, desc, file_path, sigs, commit_index):
    commit_ids = []
    for index in range(commit_index, len(commit)):
        commit_id = commit.loc[index]['id']
        if commit_id != '76761ff119acab82da016358d13b8e4bb4c6c34d':
            continue
        diff = commit.loc[index]['diff']
        commit_type, delete_sent = get_commit_type(diff, desc, file_path, sigs)
        if len(commit_type) != 0:
...
def get_commit_type(diff, desc, path, sigs):
    commit_types = []
    delete_sents = []
    diff_snippets_type = []
    ...
    for diff_snippet in diff_snippets:
        ...
        if len(add_common_sent) != 0:
            ...
            if desc == add_common_sent and delete_sent is None:
                ...
                diff_snippets_type.append(diff_snippet)
    ...
    if len(commit_types) == 0:
        ...
        def_snippets = get_def_snippets(diff_snippets, sigs)
        for i in range(len(diff_snippets_type)):
            for snippet in def_snippets:
                if diff_snippets_type == snippet:
...
def longest_common_sequence(str_one, str_two, case_sensitive=True):
    """
...
if __name__ == '__main__':
    ...
    save_path = 'D:\\MyDocument\\performance\\git_log\\auto_evol\\auto_evol\\auto_evlo18_filter.csv'
