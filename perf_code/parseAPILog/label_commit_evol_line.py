...
def main(caveat, root_path, kws):
    ...
    for index, row in caveat.iterrows():
        ...
        commit = get_line_diff(file_path, v, repo_path, line_start, line_end)
        ...
        while index < len(commit):
            ...
            commit_type, delete_sent, index = filter_id(commit, desc, file_path, sigs, index)
            commit_id = commit.loc[index]['id']
            # print(commit['id'])
...
class FormatDesc:
    ...
    def set_format_desc(self):
        ...
        for sen in sens:
            ...
            sen = sen.replace('O(n\\\\)', 'O(n!)').strip()  # .replace('\\n', '\n')
        ...
        if order_line_desc == 0 and sent_count > 1:
            ...
            if len(fw) > 0:
                ...
                format_desc = tokenizer.tokenize(format_desc.replace('O(n!)', 'O(n\\\\)'))[-1].replace('O(n\\\\)', 'O(n!)')
        ...
        # line_desc 在line的头部， line中只有1个句子，则需要添加line_desc的上文和下文
            ...
            if len(bk) > 0:
                ...
                format_desc = tokenizer.tokenize(format_desc.replace('O(n!)', 'O(n\\\\)'))[0].replace('O(n\\\\)', 'O(n!)')
        ...
        # format_desc = ' '.join([i for i in format_desc.strip().split(' ') if i != ''])
    ...
    def back(line_index, lines, line_desc):
        ...
        while line_index < len(lines) - 1:
            ...
            if re.match(r'^\* |^- |^#|^\'\'\'|^\"\"\"|>>>|---|===|\S+ :[^\S]', str(line).strip()):  # |\S+:^[A-Z]|
                break
            ...
            sens = tokenizer.tokenize(line)
            format_desc = format_desc + ' ' + sens[0].replace('O(n\\\\)', 'O(n!)').strip()  # .replace('\\n', '\n')
    ...
    def forward(line_index, lines, line_desc):
        ...
        if re.match(r'^\* |^- |^#|^\'\'\'|^\"\"\"|>>>|---|===|\S+ :[^\S]', str(line_desc).strip()):  # |\S+:^[A-Z]|
            return format_desc

        while line_index > 0:

            # 如果desc以'大写字母','*',"'''",'#','-','"""'开始的，则表明该desc头部已经结束，直接返回    ^[A-Z]|
            if re.match(r'^\* |^- |^#|^\'\'\'|^\"\"\"|>>>|---|===|\S+ :[^\S]', str(format_desc).strip()):  # |\S+: ^[A-Z]|
                break
            ...
            line = line.replace('O(n!)', 'O(n\\\\)')  # .replace('"""', '').replace('\n', '\\n')
            sens = tokenizer.tokenize(line)
            format_desc = sens[-1].replace('O(n\\\\)', 'O(n!)').strip() + ' ' + format_desc  # .replace('\\n', '\n')
...
class CommitType:
    ...
    def set_commit_type(self, sig, desc, file_path):
        ...
        for diff_snippet in diff_snippets:
            ...
            # add_common_sent = ' '.join([i for i in add_common_sent.strip().split(' ') if i != ''])
            # delete_common_sent = ' '.join([i for i in delete_common_sent.strip().split(' ') if i != ''])
            ...
            if add_common_sent == delete_common_sent:
                ...
                if desc.lstrip() != add_common_sent.lstrip():
                    ...
                    # delete_sent_ = ' '.join([i for i in delete_sent.strip().split(' ') if i != ''])
                    ls = levenshtein(delete_sent.encode('utf-8').decode('unicode-escape'),
                                     desc.encode('utf-8').decode('unicode-escape'))
                    if ls/len(desc) <= 0.58 or len(lc_subword) >= 4:  # or 1 < ls/len(desc) <= 1.72
                        commit_type = 'alter'
                    else:
                        delete_sent = ''
                        commit_type = 'add'
    ...
    def get_common_sent(alter_list, desc, blank_list):
        ...
        while alter_index < len(alter_lines):
            ...
            line = line.replace('O(n!)', 'O(n\\\\)')
            line_sents = tokenizer.tokenize(line)
            for sen_index in range(len(line_sents)):
                line_sents[sen_index] = line_sents[sen_index].replace('O(n\\\\)', 'O(n!)')
            ...
            if line_sen != '':
                ...
                for i in range(alter_index + 1, len(alter_lines)):
                    line = alter_lines[i].replace('O(n!)', 'O(n\\\\)')
                    line_sens = tokenizer.tokenize(line)
                    tag = True
                    for sen in line_sens:
                        sen = sen.replace('O(n\\\\)', 'O(n!)')
                        if sen in desc2:
    ...
    def get_delete_sent(desc, delete, delete_space, file_path):
        ...
        for index in range(len(delete)):
            ...
            delete_line = delete[index][1:].replace('O(n!)', 'O(n\\\\)')
            delete_sents = tokenizer.tokenize(delete_line)

            # 计算每个句子与desc的最长公共子序列
            for sent in delete_sents:
                sent = sent.replace('O(n\\\\)', 'O(n!)')
    ...
    def get_api_snippets(diff_snippets, sig):
        ...
        for index in range(len(diff_snippets)):
            api_line = []
            for line in diff_snippets[index][1:]:
                for api in apis_name:
                    if re.search('def '+api+'\(|class '+api+'\(', line):
                        api_line.append(line)
                        break
                        api_snippets_dic[index] = diff_snippets[index]
            # api_line = [line for line in diff_snippets[index][1:] for api in apis_name if re.search('def '+api+'\(|class '+api+'\(', line)]
            if len(api_line) != 0:
                api_snippets_dic[index] = diff_snippets[index]
...
def filter_id(commit, desc, file_path, sig, commit_index):
    """
    找到所有增加了desc的commit
    ...
    for index in range(commit_index, len(commit)):
        commit_id = commit.loc[index]['id']
        # if commit_id != '34816ae08ec2af333f47b89ae6a9cdf74686d675':
        #     continue
        diff = commit.loc[index]['diff']
        ...
        if len(commitType.commit_type) != 0:
            # commit_ids.append(commitType.commit_type, commitType.delete_sent, index, commitType.diff_snippet))
...
def get_line_diff(file_path, version, repo_path, line_start, line_end):
    """
...
if __name__ == '__main__':
    path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\userguide_format_desc6.csv'
    clone_path = 'D:\\MyDocument\\performance\\clone'
    save_path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\userguide_evol\\userguide_evo_line.csv'
