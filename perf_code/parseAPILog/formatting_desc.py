...
def reade_csv(file_path):
    ...
    :return:
    """
    df = pd.read_csv(file_path,  dtype={'lineno': int})
...
def get_sen_desc(desc):
    ...
    desc = desc.replace('O(n!)', 'O(n\\\\)')  # .replace('"""', '').replace('\n', '\\n')
    sens = tokenizer.tokenize(desc)

    if len(sens) == 1:
        return desc.replace('O(n\\\\)', 'O(n!)')  # .replace('\\n', '\n')
...
def get_one_line_desc(desc, line_no, file_path):
    ...
    for line_sen in line_sens:
        line_sen = line_sen.replace('O(n\\\\)', 'O(n!)')  # .replace('\\n', '\n')
...
if __name__ == "__main__":
    file_path = 'D:\\MyDocument\\performance\\git_log\\commit\\docstring_commits_unique2.csv'
    save_path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\docstring_format_desc11.csv'
    root_path = 'D:\\MyDocument\\performance\\download'
    caveat = reade_csv(file_path)
    data = main(caveat, root_path)
    pandas.DataFrame.to_csv(data, save_path)
