...
GIT_FIlE_CHANGE_LOG = r'git -C {} log --stat -1 {} {}> {}'
...
def main(caveat, root_path, kws):
    ...
    for row in range(len(caveat)):
        ...
        file_path = repo_path + '\\' + sub_path.replace('/', '\\')
        v = version[repo]

        commit = get_file_diff(file_path, version=v, repo_path=repo_path)
        deficiency_diff, line_diff, deficiency_ids, line_ids = get_line_diff(repo_path, line_start, line_end, caveat['filepath'][row])
        ...
        while index < len(commit):
            ...
            if commit_type == 'alter':
                ...
                new_path_list = get_new_path2(commit_id, repo_path, file_path)
                ...
                if len(new_path_list) == 0:
                    ...
                    if not tag_extral and len(deficiency_diff) != 0 and is_rename:
                        # traver_id = [commit.loc[i]['id'] for i in range(0, index + 1)]
                        diff = pd.DataFrame()
                        for id in [id for id in deficiency_ids if id not in traver_id]:
                            diff = diff.append(deficiency_diff.loc[deficiency_diff['id'] == id], ignore_index=True)
                        index = -1
                        tag_extral = True

                    elif not tag_extral and len(line_diff) != 0 and is_rename:
                        # traver_id = [commit.loc[i]['id'] for i in range(0, index + 1)]
                        diff = pd.DataFrame()
                ...
                elif index == len(commit) - 1 and not tag_extral and len(deficiency_diff) != 0 and is_rename:
                ...
                elif index == len(commit) - 1 and not tag_extral and len(line_diff) != 0 and is_rename:
                    # traver_id = [commit.loc[i]['id'] for i in range(0, index + 1)]
                    diff = pd.DataFrame()
                    for id in [id for id in line_ids if id not in traver_id]:
                        diff = diff.append(line_diff.loc[line_diff['id'] == id], ignore_index=True)
...
def get_line_diff(repo_path, line_start, line_end, file_path):
    ...
    git_file_log = GIT_FILE_LOG.format(repo_path, file_path, temp_path)
    ...
    file_ids = get_commit_id(git_checkout, git_file_log, temp_path)
    ...
    # 得到文件改名后，line_ids中的diff
    index = 0
    while index < len(line_ids):
        comm = GIT_ID_LOG.format(repo_path, line_ids[index], line_path, os.getcwd() + '\\tem_log2' + '.txt')
        os.system(comm)
    ...
    while index < len(deficiency_id):
        ...
        with open(os.getcwd() + '\\tem_log2' + '.txt', 'r', encoding='UTF-8') as f:
            lines = f.readlines()
        if len(lines) == 0:
            if deficiency_path == get_new_path(deficiency_id[index:len(deficiency_id)], repo_path, deficiency_path):
                index = index
            else:
                deficiency_path = get_new_path(deficiency_id[index:len(deficiency_id)], repo_path, deficiency_path)
                index = index - 1
        else:
            df = split_log(lines)
    ...
    return defficiency_diff, line_diff, deficiency_id, line_ids
...
def get_new_path2(id, repo_path, file_path):
...
def filter_id(commit, desc, file_path, sigs, commit_index, lines_ids):
    ...
    for index in range(commit_index, len(commit)):
        commit_id = commit.loc[index]['id']
        # if commit_id != 'c3c60f0d7a782cd429e3d7115a99cdc068a6d528':
        #     continue
        diff = commit.loc[index]['diff']
...
def get_commit_type(diff, desc, file_path, sigs):
    ...
    for diff_snippet in diff_snippets:
        ...
        if len(add_common_sent) != 0:
            ...
            # if re.match('\s+[A-Z]', add_common_sent):
            #     add_common_sent = add_common_sent.lstrip()
            if desc.lstrip() == add_common_sent.lstrip() and delete_sent is None:
                commit_type = 'add'
...
def find_delete_sent(desc, delete, delete_space, file_path):
    ...
    # if len(delete_string) > 0 and (ls/len(desc) <= 0.58 or len(lc_subword) >= 4):
    #     return delete_string
    else:
        return None
...
if __name__ == '__main__':
    path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\docstring_format_desc4.csv'
    clone_path = 'D:\\MyDocument\\performance\\clone'
    save_path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\auto_evol\\docstring_unique4.csv'
