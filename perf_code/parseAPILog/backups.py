...
def main(caveat, root_path, kws):
    ...
    for row in range(len(caveat)):
        ...
        sub_path = caveat['filepath'][row].split('/')
        ...
        deficiency_diff, line_diff, line_ids = get_line_diff(repo_path, line_start, line_end, caveat['filepath'][row])
        ...
        while index < len(commit):
            ...
            if commit_type == 'alter':
                ...
                if commit_type == 'add' and not is_rename:
                    break
...
def filter_id(commit, desc, file_path, sigs, commit_index, lines_ids):
    ...
    for index in range(commit_index, len(commit)):
        commit_id = commit.loc[index]['id']
        # if commit_id != '3a52e0a38cb8ccf20a71a03e9adbf1f0714bc9e5':
        #     continue
        diff = commit.loc[index]['diff']
...
def get_commit_type(diff, desc, file_path, sigs):
    ...
    for diff_snippet in diff_snippets:
        ...
        if len(add_common_sent) != 0:
            ...
            if desc == add_common_sent.lstrip() and delete_sent is None:
                commit_type = 'add'
...
if __name__ == '__main__':
    path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\desc_unique2.csv'
    clone_path = 'D:\\MyDocument\\performance\\clone'
    save_path = 'D:\\MyDocument\\performance\\git_log\\auto_evol_0327\\auto_evol\\auto_evlo3.csv'
