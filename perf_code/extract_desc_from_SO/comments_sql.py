...
def executeSelect(cursor):
    ...
    sql = "SELECT comment_id,body FROM comments"  # SQL 查询语句
    try:
        cursor.execute(sql)
...
def parseBody(columns):
    ...
    for column in columns:
        ...
        # savecsv(kw_desc,comment_id,'G:\\Performance\\SO_Guid_Api\\stackoverflow\\SO_desc\\SO_1011\\stackoverflow_latest_comment.csv',apis)
        # for i in kw_desc:
        #     print(comment_id," : ",i)
    print(len_kw_sen)
...
def savecsv(descs, comment_id, csv_Path,apis):
    '''
    ...
    with open(csv_Path, 'a+', encoding='gb18030',newline="") as csvfilehandler:
        ...
        for desc in descs:
            writer.writerow([comment_id,desc[0],desc[1],desc[2],apis])  # [question_id,desc,kw[],codelist]
